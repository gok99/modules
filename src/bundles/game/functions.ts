...
const scene = () => context.moduleContexts.game.state.scene;
 ...
 * @hidden
 */
export function is_type(obj: GameObject, type: string): boolean {
 ...
 * @hidden
 */
export function is_any_type(obj: GameObject, types: string[]): boolean {
 ...
 */
export function array_test(x: any): boolean {
 ...
 */
export function pair(x: any, xs: any): [any, any] {
  return [x, xs];
}
 ...
 * LOW-LEVEL FUNCTION, NOT SOURCE
 */
export function is_pair(x: any): boolean {
 ...
 * LOW-LEVEL FUNCTION, NOT SOURCE
 */
export function head(xs: List): any {
  if (is_pair(xs)) {
    return xs![0];
  }
  throw new Error(
 ...
 */
export function map(f: (x: any) => any, xs: List) {
...
export function run_scene(
  preload: () => void,
  create: () => void,
  update: () => void,
) {
  context.moduleContexts.game.state.renderPreview = true;
  context.moduleContexts.game.state.lifecycleFuncs = {
    preload,
    create,
    update,
  };
...
export function add(obj: GameObject): GameObject | undefined {
  if (is_any_type(obj, ObjTypes)) {
    context.moduleContexts.game.state.scene.add.existing(get_game_obj(obj));
    return obj;
  }
...
export function create_image(
  ...
  if (
    ...
    const image = new Phaser.GameObjects.Sprite(context.moduleContexts.game.state.scene, x, y, asset_key);
